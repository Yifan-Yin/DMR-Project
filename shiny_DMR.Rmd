...
ui <- fluidPage(
  ...
  sidebarPanel(
           ...
           p('Here, you can view the methylation at any CpG covered by the Illumina 
                EPIC methylation array. We have available Villi, Trophoblast, Hofbaeur cells, 
                Stromal and Endothelial cell populations.'),
           hr(),
           ...
           p('After selecting a gene above, you can select specific associated-CpGs to display as 
             boxplots'),
           ...
           checkboxGroupInput('checkbox', 
                              'Information in table to show', 
                              names(anno1), 
                              selected = names(anno1)[-10:-12])
  ...
  mainPanel(
    ...
    # INDIVIDUAL CPG PLOT
    plotOutput('boxplot_individual', height = '650px')
...
server <- function(input, output, session) {
    ...
    observeEvent(input$info_cell_clicked, {c_info = input$info_cell_clicked
    ...
    observeEvent(input$submit2, {output$info <- DT::renderDataTable(DT::datatable(cpg_info_total(cpg_name = event_submit(), gene_name = event_submit2())[,input$checkbox, drop = FALSE], options = list(pageLength = 5),
        selection = 'single',
        rownames = FALSE)%>% formatStyle(1, cursor = 'pointer', color = 'blue'))
    })
    #reset button
