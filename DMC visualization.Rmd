...
basename <- file.path("Z:", "Victor", "Projects", "NIH - cells", "data", "main", "interim")

anno <- readRDS(file.path(basename, '3_1_annotation.rds'))
ref_group <- anno[, c(1,12:19)]
anno <- anno[, -12:-19]

#mean and SD data
stat <- readRDS(file.path(basename, '3_1_betas_cell_mean_sd.rds'))

#read cpg methylation data
beta <- readRDS(file.path(basename, '1_4_betas_noob_filt_XYkeep.rds'))

#pData
pDat <- readRDS(file.path(basename, '2_3_pDat_contam.rds'))
pDat <- pDat %>%
...
#color code
color_code <- readRDS(file.path(basename, '2_3_color_code.rds'))
