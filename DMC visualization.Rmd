...
gene <- str_split(anno$genes_symbol, ', ') %>%
  unlist() %>%
  unique() %>%
  na.omit() %>%
  sort()
...
stat_plot <- function(cpg_name = NULL, gene_name = NULL){
  ...
  if (!is.null(gene_name) & is.null(cpg_name)){
    ...
    } else if (!is.null(cpg_name) & is.null(gene_name)){
  ...
  # combine tracks plot
  plot <- ggarrange(p1, p2, p3, ncol = 1, heights = c(p1_h-(n_trans-7), 7, n_trans))
  plot
  }
