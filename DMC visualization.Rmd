...
boxplot_trimester <- function(cpg){
  ggplot(cpg, aes(x=Trimester, y = betas, fill = Tissue)) +
...
plot_cpgs_in_gene <- function(gene, font_size = 12, point_size = 1) {
  ...
  p3 <- betas_gene %>% 
    ...
    facet_wrap(vars(group), ncol = 1) +
    guides(fill=guide_legend(ncol=2, override.aes = list(alpha=0.7, color = 'black'))) +
    theme_bw(base_size = font_size) +
