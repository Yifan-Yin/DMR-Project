...
boxplot_trimester_single <- function(cpg_name){
    ...
    #boxplot
    boxplot_trimester_individual(cpg)
}
```
...
boxplot_tissue <- function(cpg, nrow = 3){
  ...
  {ggplot(cpg, aes(x=Tissue, y=betas, fill = Tissue)) +
      ...
      labs(x = 'Tissue', y ='betas') +scale_fill_manual(values = colors) + scale_color_manual(values = colors) + ylim(0,1)
  }
  else
  {
    ggplot(cpg, aes(x=Tissue, y=betas, fill = Tissue)) +
...
boxplot_trimester <- function(cpg){
  ...
  ggplot(cpg, aes(x=cpg, y=betas, fill = cpg)) +
      geom_violin()+
      geom_jitter(aes(color = Trimester), alpha = 0.5, show.legend = F) +
      facet_wrap_paginate(.~Trimester)+ 
      labs(x = 'Trimester', y ='betas')  +ylim(0,1)+ theme(legend.position="none",axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
  
}

boxplot_tissue_individual <- function(cpg){
  ggplot(cpg, aes(x=Tissue, y=betas, fill = Tissue)) +
      geom_violin()+
      geom_jitter(aes(color = Tissue), alpha = 0.5, show.legend = F) +
      facet_wrap_paginate(.~cpg)+ 
...
boxplot_trimester_individual <- function(cpg){
  ggplot(cpg, aes(x=Trimester, y=betas, fill = Trimester)) +
      geom_violin()+
      geom_jitter(aes(color = Trimester), alpha = 0.5, show.legend = F) +
      facet_wrap(.~cpg)+ 
      labs(x = cpg$cpg, y ='betas')  +ylim(0,1)
}
```
